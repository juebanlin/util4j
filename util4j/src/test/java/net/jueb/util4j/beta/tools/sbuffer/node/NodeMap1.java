 ...
 * @author juebanlin
 */
public class NodeMap1<K,V> implements RouteMap<K, V>{
	...
	public NodeMap1(MaskEnum mask,boolean byLeft) {
		this.byLeft=byLeft;
...
//		private final DataNode<T>[] sub;
		private int index;
		private DataNode<T> left;//横向
		private DataNode<T> down;
...
//			sub=new DataNode[getNodeSize()];
		...
		public DataNode<T> getSub(int index)
		{
//			return sub[index];
			DataNode<T> node=left;
			while(node!=null)
			{
				if(node.index==index)
				{
		...
		public void setSub(int index,DataNode<T> newNode)
		{
//			sub[index]=node;
			DataNode<T> node=left;
			DataNode<T> pre=null;
	...
	public static class Test{
		...
		NodeMap1<Integer,Byte> nmap=new NodeMap1<>();
