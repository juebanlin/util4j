 ...
 * @author juebanlin
 */
public class NodeMap6<K,V> implements RouteMap<K, V>{
	...
	public NodeMap6(MaskEnum mask,boolean byLeft) {
		this.byLeft=byLeft;
...
//		int mask=config.mask;
	...
	private class DataEntry<K, V> implements Entry<K, V>
	{
		private final K key;
	...
	protected V find(Node<V> currentNode,int layout,int pos,int number)
	{
		if(layout==0)
		{
		...
		V value=null;
		for(int i=pos;i<sub.length;i++)
		{
			Node<V> node=sub[pos];
			value=find(node, layout, pos,number);
			if(value!=null)
			{
		...
		{
			pos++;//当前层下一个去往底层的入口
			value=find(currentNode, layout, pos,number);
		}
		layout--;
		return find(node, layout,0,number);
	}
	...
	public static class Test{
		...
		NodeMap6<Integer,Byte> nmap=new NodeMap6<>();
