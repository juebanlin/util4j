package net.jueb.util4j.beta.tools.log.uiLog.old;
...
public class UiLog {
	...
	//日志数据输出流==存储日志的内存字节数组
	private UiOutPutStream logDataStream;
	private OutputStream os;
	
	//默认日志格式
	private  String defaultPattern="UiLog==>[%p]:%l{%m}%n";
	private Layout lay;
	 ...
	 * @param log
	 */
	public UiLog(Logger log,JTextComponent textUi) {
		this.textUi=textUi;
		this.log=log;
		//初始化内存数组
		logDataStream=new UiOutPutStream(this.textUi);
		os=logDataStream;
		//设置日志格式
	 ...
	 * @param pattern
	 */
	public UiLog(Logger log,JTextComponent textUi,String pattern) {		
	...
	class UiOutPutStream extends ByteArrayOutputStream
	{
		private JTextComponent textUi;
