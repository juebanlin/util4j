...
import net.jueb.util4j.serializable.nmap.falg.Flag;
...
public class NUTF16LEString extends NType<String>{
	...
	public NUTF16LEString(String obj) {
		super(obj, Flag.Head.NUTF16LEString, Flag.End.NUTF16LEString);
	}
		...
		if(checkHead(ti))
		{
			...
			{//最大要读取的字节次数
				...
				{//如果缓存满了，则判断是否是尾巴标记
					...
					if(isEnd && (body.size()%2)==0)
					{//如果是end,则返回字符串
						//UTF16LE一个字占2个字节所以要判断body.size()%2
						return new NUTF16LEString(new String(body.toByteArray(),"UTF-16LE"));
					}
